...
const SkillFeedback = ({ feedback }) => {
  ...
  const { keywordsFound = [], missingSkills = [], bonusSkills = [] } = feedback;
    ...
    <div className="flex flex-col gap-6 bg-white border border-[#EBEAE4] p-6 rounded-2xl shadow-sm">
      ...
      <div>
        ...
        {keywordsFound.length > 0 ? (
          ...
          <p className="text-xs text-slate-400">No matched skills found.</p>
        )}
      </div>
      ...
      <div>
        ...
        {missingSkills.length > 0 ? (
          ...
          <p className="text-xs text-slate-400">No missing skills — great match!</p>
        )}
      </div>
      ...
      <div>
        ...
        {bonusSkills.length > 0 ? (
          <div className="flex flex-wrap gap-1.5">
            {bonusSkills.map((bs, i) => (
              <span key={i} className="text-xs font-medium bg-amber-50 text-amber-700 border border-amber-100 px-2.5 py-1 rounded-md">
                {bs}
              </span>
            ))}
          </div>
        ) : (
          <p className="text-xs text-slate-400">No bonus skills detected.</p>
        )}
      </div>
